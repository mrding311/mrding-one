...
/**
 * Created by DINGSHUAI685 on 2015-10-16.
 */
public class ZkDemo implements Watcher {

    public static final Logger LOG = Logger.getLogger(ZkDemo.class);
    ...
    public void process(WatchedEvent watchedEvent) {
        LOG.info("收到事件通知：" + watchedEvent.toString() );
    ...
    public static void main(String [] args){
        ...
        // 定义父子类节点路径
        String rootPath = "/esales-collect";
        ...
        // 连接zk服务器
        demo.connectionZookeeper("10.20.8.161:2181,10.20.8.126:2181,10.20.8.175:2181");

        // 创建节点数据
//        if ( demo.createPath( rootPath, "<父>节点数据" ) ) {
//            LOG.info( "节点[" + rootPath + "]数据内容[" + demo.readData( rootPath ) + "]" );
//        }
        // 创建子节点, 读取 + 删除
        if ( demo.createPath( child1Path, "<父-子(1)>节点数据" ) ) {
            LOG.info( "节点[" + child1Path + "]数据内容[" + demo.readData( child1Path ) + "]" );
            demo.deletePath(child1Path);
            LOG.info( "节点[" + child1Path + "]删除值后[" + demo.readData( child1Path ) + "]" );
        }

        // 创建子节点, 读取 + 修改
        if ( demo.createPath( child2Path, "<父-子(2)>节点数据" ) ) {
            LOG.info( "节点[" + child2Path + "]数据内容[" + demo.readData( child2Path ) + "]" );
            demo.writeData( child2Path, "<父-子(2)>节点数据,更新后的数据" );
            LOG.info( "节点[" + child2Path+ "]数据内容更新后[" + demo.readData( child2Path ) + "]" );
        }

        // 获取子节点
        List<String> childPaths = demo.getChild(rootPath);
        if(null != childPaths){
            LOG.info( "节点[" + rootPath + "]下的子节点数[" + childPaths.size() + "]" );
        ...
        // 判断节点是否存在
        LOG.info( "检测节点[" + rootPath + "]是否存在:" + demo.isExists(rootPath)  );
        LOG.info( "检测节点[" + child1Path + "]是否存在:" + demo.isExists(child1Path)  );
        LOG.info( "检测节点[" + child2Path + "]是否存在:" + demo.isExists(child2Path)  );


        demo.releaseConnection();
    }
